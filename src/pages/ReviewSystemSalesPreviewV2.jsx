export default function ReviewSystemSalesPreviewV2() {
  const bonusMaterials = [
    {
      title: "90-Day Review Acceleration Plan",
      description: "Phase-by-phase implementation roadmap with weekly targets and accountability checkpoints",
      icon: "📅"
    },
    {
      title: "Sales Meeting Plans for Review Leverage",
      description: "12 ready-to-use meeting agendas with talking points, role-plays, and team exercises",
      icon: "👥"
    },
    {
      title: "Review Capture Scripts - Manager Edition",
      description: "Manager-focused scripts for coaching, accountability conversations, and team training",
      icon: "📋"
    },
    {
      title: "Review Capture Scripts - Salesperson Edition",
      description: "Complete script library for consultations, post-install calls, texts, and follow-ups",
      icon: "💬"
    },
    {
      title: "Review Request Text Templates",
      description: "Copy-paste text message templates for every stage of the review capture process",
      icon: "📱"
    },
    {
      title: "Review Catalog Playbook",
      description: "System for organizing your best reviews by theme to use in sales conversations",
      icon: "📚"
    },
    {
      title: "Advanced Scripts & Role-Play Scenarios",
      description: "Challenging customer scenarios with proven response frameworks and practice exercises",
      icon: "🎭"
    },
    {
      title: "Social Media Efficiency Guide",
      description: "Repurpose reviews into social content without adding work to your plate",
      icon: "📲"
    },
    {
      title: "Overcoming Barriers to Implementation",
      description: "Solutions to the 10 most common obstacles teams face when rolling out review systems",
      icon: "🚧"
    },
    {
      title: "Weekly Sales Meeting Dashboard Template",
      description: "Track team performance, celebrate wins, and identify coaching opportunities",
      icon: "📊"
    }
  ];

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <header className="bg-gradient-to-br from-primary-700 to-primary-900 text-white py-12 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <div className="inline-block bg-yellow-400 text-gray-900 px-4 py-1 rounded-full text-sm font-bold mb-4">
            🔥 COMPLETE PACKAGE PREVIEW
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            Review Leverage System Complete Package
          </h1>
          <p className="text-2xl text-primary-100 mb-3">
            Everything You Need to Build a Review-Driven Lead Generation Engine
          </p>
          <p className="text-lg text-primary-200 max-w-3xl mx-auto">
            2 Core Manuals + 10 Implementation Resources • Proven System • Cut Marketing Spend 64% • Generate 70% Organic Leads
          </p>
        </div>
      </header>

      {/* Main Content */}
      <main className="max-w-6xl mx-auto px-4 py-12">
        
        {/* Value Proposition */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8 border-t-4 border-primary-700">
          <h2 className="text-3xl font-bold text-gray-900 mb-4 text-center">
            The Only Review System Built BY a Flooring Dealer FOR Flooring Dealers
          </h2>
          <p className="text-xl text-gray-700 text-center mb-6 max-w-4xl mx-auto">
            This isn't generic marketing advice. It's the exact system I used to reduce marketing spend from <strong>$110,000 to $40,000</strong> while growing sales <strong>70%</strong> and generating <strong>70% of leads organically</strong>.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
            <div className="text-center">
              <div className="text-4xl font-bold text-primary-700 mb-2">64%</div>
              <div className="text-gray-600 font-semibold">Marketing Cost Cut</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-bold text-primary-700 mb-2">70%</div>
              <div className="text-gray-600 font-semibold">Organic Lead Flow</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-bold text-primary-700 mb-2">3x</div>
              <div className="text-gray-600 font-semibold">Digital Leads</div>
              <div className="text-xs text-gray-500 mt-1">(Over 6-12 months)</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-bold text-primary-700 mb-2">4.9★</div>
              <div className="text-gray-600 font-semibold">Rating Achieved</div>
            </div>
          </div>
        </div>

        {/* Core Manuals */}
        <div className="mb-12">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">
            📖 Core System Manuals
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Manual 1: Owner/Manager */}
            <div className="bg-white rounded-lg shadow-lg overflow-hidden border-2 border-primary-200">
              <div className="bg-gradient-to-br from-primary-700 to-primary-900 text-white p-6">
                <div className="text-4xl mb-3">📊</div>
                <h3 className="text-2xl font-bold mb-2">
                  The Ultimate Leverage System
                </h3>
                <p className="text-primary-100">For Owners & Managers</p>
              </div>
              <div className="p-6">
                <p className="text-gray-700 mb-4 font-semibold">
                  Complete marketing leverage manual with ROI analysis, team accountability systems, and 90-day implementation plan.
                </p>
                <ul className="space-y-2 mb-6 text-sm text-gray-600">
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Marketing math & cost reduction strategies</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>4-touch capture system with team tracking</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Compensation structures for accountability</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Negative review protocol & damage control</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Google Business Profile optimization</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Real-world data from Matt's store ($4.57M → $7.3M)</span>
                  </li>
                </ul>
                <div className="bg-primary-50 border-2 border-primary-200 rounded-lg p-4 text-center">
                  <div className="text-gray-700 font-semibold">📄 143 Pages</div>
                  <div className="text-sm text-gray-600 mt-1">Comprehensive Implementation Guide</div>
                </div>
              </div>
            </div>

            {/* Manual 2: Salesperson */}
            <div className="bg-white rounded-lg shadow-lg overflow-hidden border-2 border-green-200">
              <div className="bg-gradient-to-br from-green-600 to-green-700 text-white p-6">
                <div className="text-4xl mb-3">💰</div>
                <h3 className="text-2xl font-bold mb-2">
                  The Salesperson's Guide
                </h3>
                <p className="text-green-100">Turn Reviews Into Commissions</p>
              </div>
              <div className="p-6">
                <p className="text-gray-700 mb-4 font-semibold">
                  Salesperson-focused guide showing how Google reviews directly increase commissions through warmer leads and higher close rates.
                </p>
                <ul className="space-y-2 mb-6 text-sm text-gray-600">
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Why reviews = bigger paychecks (with real data)</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>4-step system (only 3 minutes per customer)</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>How to use reviews to close more sales</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Track impact on your personal commissions</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>Overcoming customer objections to leaving reviews</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-green-600 mt-0.5">✓</span>
                    <span>30-day challenge to build the habit</span>
                  </li>
                </ul>
                <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4 text-center">
                  <div className="text-gray-700 font-semibold">📄 34 Pages</div>
                  <div className="text-sm text-gray-600 mt-1">Sales-Focused Action Guide</div>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Realistic Results Timeline */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8 border-l-4 border-primary-700">
          <h2 className="text-3xl font-bold text-gray-900 mb-4 text-center">
            📅 What to Expect: Realistic Timeline
          </h2>
          <p className="text-center text-gray-600 mb-8 max-w-3xl mx-auto">
            This isn't a "get rich quick" scheme. It's a proven system with predictable, compounding results.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* First 90 Days */}
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6 border-2 border-blue-300">
              <div className="text-center mb-4">
                <div className="text-4xl mb-2">🌱</div>
                <h3 className="text-xl font-bold text-gray-900">First 90 Days</h3>
                <div className="text-sm text-blue-700 font-semibold">Foundation Phase</div>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-start gap-2">
                  <span className="text-blue-600 mt-0.5">✓</span>
                  <span>30-60 new reviews captured (typical for dealers following the system consistently)</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-600 mt-0.5">✓</span>
                  <span>10-20% initial lead growth</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-600 mt-0.5">✓</span>
                  <span>Team trained & system deployed</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-blue-600 mt-0.5">✓</span>
                  <span>Google ranking starts improving</span>
                </li>
              </ul>
            </div>

            {/* Months 6-12 */}
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6 border-2 border-green-300">
              <div className="text-center mb-4">
                <div className="text-4xl mb-2">📈</div>
                <h3 className="text-xl font-bold text-gray-900">Months 6-12</h3>
                <div className="text-sm text-green-700 font-semibold">Growth Phase</div>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span>Digital leads (web forms, chat) triple (3x growth)</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span>Total lead volume increases 30-40% (all sources: digital, phone calls, walk-ins)</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span>Marketing spend starts dropping significantly</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span>System runs on autopilot with weekly check-ins</span>
                </li>
              </ul>
            </div>

            {/* Year 1 Impact */}
            <div className="bg-gradient-to-br from-yellow-50 to-orange-100 rounded-lg p-6 border-2 border-yellow-400">
              <div className="text-center mb-4">
                <div className="text-4xl mb-2">🚀</div>
                <h3 className="text-xl font-bold text-gray-900">Year 1 Impact</h3>
                <div className="text-sm text-orange-700 font-semibold">Proven Results</div>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-start gap-2">
                  <span className="text-orange-600 mt-0.5">✓</span>
                  <span><strong>$30K-$70K</strong> annual ad savings</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-orange-600 mt-0.5">✓</span>
                  <span><strong>$50K-$100K+</strong> revenue growth</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-orange-600 mt-0.5">✓</span>
                  <span>70% of leads from organic search (not paid ads)</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-orange-600 mt-0.5">✓</span>
                  <span>System becomes a permanent business asset</span>
                </li>
              </ul>
            </div>
          </div>

          <div className="mt-6 bg-primary-50 border-2 border-primary-300 rounded-lg p-4">
            <p className="font-semibold text-gray-900 text-center mb-2">
              💡 These timelines are based on Matt's actual results and consistent feedback from dealers implementing this system.
            </p>
            <p className="text-sm text-gray-600 text-center italic">
              *Note: "Digital leads" = website forms, chat, and Google Local Services clicks. "Total lead volume" includes all sources: digital, phone calls, walk-ins, and referrals.*
            </p>
          </div>
        </div>

        {/* Bonus Materials Section */}
        <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg p-8 mb-8 border-2 border-yellow-300">
          <div className="text-center mb-8">
            <div className="inline-block bg-yellow-400 text-gray-900 px-4 py-2 rounded-full font-bold mb-4">
              🎁 BONUS: 10 Implementation Resources
            </div>
            <h2 className="text-3xl font-bold text-gray-900 mb-3">
              Everything You Need to Execute Immediately
            </h2>
            <p className="text-xl text-gray-700 max-w-3xl mx-auto">
              Don't just learn the system—implement it today with ready-to-use templates, scripts, and tools. These resources are worth $1,500+ if purchased separately.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {bonusMaterials.map((bonus, index) => (
              <div key={index} className="bg-white rounded-lg p-5 shadow border-2 border-yellow-200">
                <div className="flex items-start gap-4">
                  <div className="text-3xl flex-shrink-0">{bonus.icon}</div>
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-900 mb-1">{bonus.title}</h4>
                    <p className="text-sm text-gray-600">{bonus.description}</p>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* What You're Really Paying For */}
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg shadow-xl p-8 mb-8 border-2 border-purple-300">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">
            💎 What You're Really Paying For
          </h2>
          
          <div className="max-w-4xl mx-auto space-y-4 text-gray-700 text-lg">
            <p>
              This system represents <strong>years of trial and error, hundreds of hours of refinement, and countless mistakes</strong> I made so you don't have to.
            </p>
            
            <p>
              This isn't a system I built in 6 months and then started selling. I've been refining this for <strong>6+ years</strong> while growing a real flooring business from $4.57M to $7.3M. Every script, every accountability structure, every tracking dashboard—it all comes from <strong>real-world trial and error with real money on the line.</strong>
            </p>
            
            <div className="bg-white border-2 border-purple-300 rounded-lg p-6 my-6">
              <p className="font-bold text-gray-900 text-xl mb-2">You're not buying a PDF.</p>
              <p className="text-gray-700">
                You're buying <strong>years of my life condensed into a proven, replicable system</strong> that you can implement in 90 days instead of the 3-5 years it took me to figure it out.
              </p>
              <p className="text-gray-700 mt-3">
                You're not buying theory. You're buying a battle-tested system that's already proven in a real store with real salespeople and real customers.
              </p>
            </div>
            
            <div className="text-center bg-indigo-100 border-2 border-indigo-300 rounded-lg p-4">
              <p className="font-bold text-gray-900 text-lg">
                <strong>Translation:</strong> You're paying $997 to skip 3-5 years of expensive mistakes, wasted ad spend, and trial-and-error.
              </p>
              <p className="text-indigo-700 font-semibold mt-2">That's the real value.</p>
            </div>
          </div>
        </div>

        {/* Results Require Implementation */}
        <div className="bg-orange-50 border-2 border-orange-300 rounded-lg p-6 mb-8">
          <h3 className="text-xl font-bold text-gray-900 mb-3 text-center">
            ⚠️ Results Require Consistent Implementation
          </h3>
          <p className="text-gray-700 mb-3">
            This system works—but only if you work it. That means following the scripts, holding your team accountable, and tracking metrics weekly. I'll give you everything you need, but I can't do the work for you.
          </p>
          <p className="text-gray-700">
            If you're looking for a "set it and forget it" solution, this isn't it. If you're ready to build a system and manage it like a real asset, this is exactly what you need.
          </p>
        </div>

        {/* Package Summary */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">
            📦 Complete Package Summary
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center mb-8">
            <div>
              <div className="text-5xl font-bold text-primary-700 mb-2">2</div>
              <div className="text-gray-700 font-semibold">Core Manuals</div>
              <div className="text-sm text-gray-600 mt-1">(Owner + Salesperson)</div>
            </div>
            <div>
              <div className="text-5xl font-bold text-primary-700 mb-2">10</div>
              <div className="text-gray-700 font-semibold">Bonus Resources</div>
              <div className="text-sm text-gray-600 mt-1">(Scripts, templates, tools)</div>
            </div>
            <div>
              <div className="text-5xl font-bold text-primary-700 mb-2">500+</div>
              <div className="text-gray-700 font-semibold">Pages of Content</div>
              <div className="text-sm text-gray-600 mt-1">(Everything you need)</div>
            </div>
          </div>

          {/* Pricing */}
          <div className="border-t-2 border-gray-200 pt-6">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-gray-900 mb-3">📦 Package Options</h3>
              <div className="text-4xl font-bold text-primary-700 mb-2">STARTING AT $997</div>
              <div className="text-gray-600 text-sm">(DIY Self-Implementation)</div>
              <div className="text-gray-500 text-xs mt-2 italic">(Full pricing & implementation options below)</div>
            </div>
          </div>
        </div>

        {/* Asset vs Expense */}
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg shadow-xl p-8 mb-8 border-2 border-green-300">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">
            💎 This Is an Asset, Not an Expense
          </h2>
          
          <div className="max-w-4xl mx-auto mb-8">
            <p className="text-xl text-gray-700 text-center mb-6">
              Unlike traditional marketing that disappears the moment you stop paying, this system generates leads <strong>24/7/365</strong> — forever.
            </p>
          </div>

          {/* Asset vs Expense Comparison */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Traditional Marketing (Expense) */}
            <div className="bg-red-50 border-2 border-red-300 rounded-lg p-6">
              <div className="text-center mb-4">
                <div className="text-3xl mb-2">📉</div>
                <h4 className="font-bold text-gray-900 text-lg">Traditional Marketing (Expense)</h4>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-start gap-2">
                  <span className="text-red-600 mt-0.5">✗</span>
                  <span><strong>Radio ad ($2,000):</strong> Heard once, forgotten immediately</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-red-600 mt-0.5">✗</span>
                  <span><strong>Mailer ($1,500):</strong> 1% response rate, then trash</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-red-600 mt-0.5">✗</span>
                  <span><strong>Facebook ads ($3,000/mo):</strong> Stop paying? Leads stop</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-red-600 mt-0.5">✗</span>
                  <span><strong>TV commercial ($5,000):</strong> Airs for 30 days, then gone</span>
                </li>
              </ul>
              <div className="mt-4 p-3 bg-red-100 rounded text-center">
                <p className="font-bold text-red-900">Total: $11,500+</p>
                <p className="text-xs text-red-700">And you have NOTHING permanent to show for it</p>
              </div>
            </div>

            {/* Review System (Asset) */}
            <div className="bg-green-50 border-2 border-green-400 rounded-lg p-6">
              <div className="text-center mb-4">
                <div className="text-3xl mb-2">📈</div>
                <h4 className="font-bold text-gray-900 text-lg">Review System (Permanent Asset)</h4>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span><strong>Works 24/7/365:</strong> No ongoing ad spend required</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span><strong>Permanent visibility:</strong> Reviews stay on Google forever</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span><strong>Compounds over time:</strong> More reviews = more leads</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-green-600 mt-0.5">✓</span>
                  <span><strong>Free organic traffic:</strong> Dominate "flooring near me" searches</span>
                </li>
              </ul>
              <div className="mt-4 p-3 bg-green-100 rounded text-center">
                <p className="font-bold text-green-900">One-Time: $997</p>
                <p className="text-xs text-green-700">And it generates leads FOREVER</p>
              </div>
            </div>
          </div>
        </div>

        {/* Real Results From Matt's Store */}
        <div className="bg-primary-900 text-white rounded-lg shadow-xl p-8 mb-8">
          <h2 className="text-3xl font-bold mb-6 text-center">
            🌟 Real Results From Matt's Flooring Store
          </h2>
          <p className="text-center text-primary-100 mb-8 max-w-3xl mx-auto text-lg">
            These aren't projections or estimates. These are real results from Matt's flooring store in McMinnville, Oregon.
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto mb-8">
            <div className="bg-primary-800 rounded-lg p-6">
              <div className="flex items-start gap-4">
                <div className="text-4xl">💰</div>
                <div>
                  <div className="text-2xl font-bold text-yellow-400 mb-2">$70,000/Year Saved</div>
                  <p className="text-primary-100">Reduced marketing spend from $110K to $40K annually</p>
                  <p className="text-sm text-primary-200 mt-2 italic">"That's $70K back in my pocket every single year."</p>
                </div>
              </div>
            </div>

            <div className="bg-primary-800 rounded-lg p-6">
              <div className="flex items-start gap-4">
                <div className="text-4xl">⭐</div>
                <div>
                  <div className="text-2xl font-bold text-yellow-400 mb-2">4.9★ Rating</div>
                  <p className="text-primary-100">Built 450+ Google reviews dominating local search</p>
                  <p className="text-sm text-primary-200 mt-2 italic">"We own 'flooring near me' in our market."</p>
                </div>
              </div>
            </div>

            <div className="bg-primary-800 rounded-lg p-6">
              <div className="flex items-start gap-4">
                <div className="text-4xl">📈</div>
                <div>
                  <div className="text-2xl font-bold text-yellow-400 mb-2">60% Revenue Growth</div>
                  <p className="text-primary-100">Grew from $4.57M to $7.3M in 6 years</p>
                  <p className="text-sm text-primary-200 mt-2 italic">"And I now work 20-30 hours/week <strong>on the flooring business</strong>, not 80+."</p>
                </div>
              </div>
            </div>

            <div className="bg-primary-800 rounded-lg p-6">
              <div className="flex items-start gap-4">
                <div className="text-4xl">⏰</div>
                <div>
                  <div className="text-2xl font-bold text-yellow-400 mb-2">18-32 Work Days/Year Reclaimed</div>
                  <p className="text-primary-100 text-sm mb-3">
                    Matt now works 20-30 hrs/week <strong>on the flooring business</strong>—but for years, he was working 80+ hour weeks, running at 12 mph when his team could only handle 5. He was overextended, burned out, and stressed.
                  </p>
                  <p className="text-primary-100 text-sm mb-2">
                    <strong>Shifting focus to building this review-driven system was one of the biggest variables that changed everything.</strong> He got his time back, his sanity back, and his business became more profitable with less chaos.
                  </p>
                  <p className="text-primary-100 text-sm mb-2">
                    <strong>Now he spends the rest of his time helping dealers like you implement these same systems—so you don't have to spend 6 years figuring it out yourself.</strong>
                  </p>
                  <p className="text-sm text-primary-200 italic">"I have a life outside of work now—and you can too."</p>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-primary-800 rounded-lg p-6 max-w-3xl mx-auto">
            <div className="flex items-start gap-4">
              <div className="text-4xl">📊</div>
              <div>
                <div className="text-2xl font-bold text-yellow-400 mb-2">52.5% Overall Close Rate</div>
                <p className="text-primary-100">(All Leads → Orders) vs. 20-35% industry average</p>
                <p className="text-sm text-primary-200 mt-2">Reviews pre-qualify leads and eliminate tire-kickers before they waste your time</p>
              </div>
            </div>
          </div>

          <div className="mt-8 text-center bg-primary-950 rounded-lg p-6">
            <p className="text-primary-100 text-lg mb-2">
              <strong className="text-white">Built by a flooring dealer. Tested in a real store. Proven with real numbers.</strong>
            </p>
            <p className="text-primary-200">
              This isn't consultant theory—it's a system I use every single day.
            </p>
          </div>
        </div>

        {/* What This Means for YOUR Store */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8 border-2 border-primary-300">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">
            💡 What This Means for YOUR Store:
          </h2>
          
          <div className="space-y-6 max-w-4xl mx-auto">
            <div className="bg-green-50 border-2 border-green-300 rounded-lg p-6">
              <h3 className="font-bold text-gray-900 text-xl mb-3">If you do $3M-$5M/year in revenue:</h3>
              <ul className="space-y-2 text-gray-700 mb-4">
                <li>• A 30% lead increase = <strong>$900K-$1.5M in additional sales opportunities</strong></li>
                <li>• Even at a 40% close rate, that's <strong>$360K-$600K in new revenue</strong></li>
                <li>• Marketing savings: <strong>$30K-$70K/year</strong> (conservatively)</li>
              </ul>
              <div className="bg-green-100 border-2 border-green-400 rounded-lg p-4 text-center">
                <p className="font-bold text-gray-900 text-lg">Your investment: $997</p>
                <p className="font-bold text-green-700 text-2xl">Your Year 1 return: $390K-$670K+</p>
              </div>
            </div>

            <div className="bg-purple-50 border-2 border-purple-300 rounded-lg p-6">
              <h3 className="font-bold text-gray-900 text-xl mb-3">If you do $5M-$10M+/year in revenue:</h3>
              <ul className="space-y-2 text-gray-700 mb-4">
                <li>• A 30% lead increase = <strong>$1.5M-$3M+ in additional sales opportunities</strong></li>
                <li>• At a 40% close rate, that's <strong>$600K-$1.2M+ in new revenue</strong></li>
                <li>• Marketing savings: <strong>$50K-$100K+/year</strong> (or more for multi-location dealers)</li>
              </ul>
              <div className="bg-purple-100 border-2 border-purple-400 rounded-lg p-4 text-center">
                <p className="font-bold text-gray-900 text-lg">Your investment: $997</p>
                <p className="font-bold text-purple-700 text-2xl">Your Year 1 return: $650K-$1.3M+</p>
              </div>
              <p className="text-sm text-gray-600 italic mt-3 text-center">
                *Note: Multi-location dealers or larger operations may benefit from the VIP package for custom implementation and on-site training.*
              </p>
            </div>

            <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-6">
              <h3 className="font-bold text-gray-900 text-xl mb-3">If you do $1M-$2M/year in revenue:</h3>
              <ul className="space-y-2 text-gray-700 mb-4">
                <li>• A 30% lead increase = <strong>$300K-$600K in additional opportunities</strong></li>
                <li>• At a 40% close rate, that's <strong>$120K-$240K in new revenue</strong></li>
                <li>• Marketing savings: <strong>$15K-$35K/year</strong></li>
              </ul>
              <div className="bg-blue-100 border-2 border-blue-400 rounded-lg p-4 text-center">
                <p className="font-bold text-gray-900 text-lg">Your investment: $997</p>
                <p className="font-bold text-blue-700 text-2xl">Your Year 1 return: $135K-$275K+</p>
              </div>
            </div>
          </div>
        </div>

        {/* Guarantee */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8 border-2 border-green-400">
          <div className="text-center">
            <div className="text-5xl mb-4">🛡️</div>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              100% Money-Back Guarantee
            </h2>
            <p className="text-lg text-gray-700 max-w-2xl mx-auto mb-3">
              If you implement this system for 90 days and don't see measurable improvement in your review capture rate or lead quality, I'll refund your investment. No questions asked.
            </p>
            <p className="text-sm text-gray-600 italic">
              I'm that confident this system works because I built it, tested it in my own store, and use it every single day.
            </p>
          </div>
        </div>

        {/* Who This Is For */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">
            🎯 Who This System Is For
          </h2>
          <p className="text-center text-gray-600 mb-8 max-w-3xl mx-auto">
            This system works best for independent flooring dealers who are ready to build a sustainable lead-generation asset.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Independent flooring dealers</strong> with under 200 Google reviews</span>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Owners tired of spending</strong> $60K-$120K/year on ads that barely break even</span>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Sales teams that struggle</strong> with consistent review capture</span>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Dealers ready to build a permanent asset,</strong> not rent leads from Google forever</span>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Businesses wanting</strong> 70% of leads from organic search (not ads)</span>
            </div>
            <div className="flex items-start gap-3">
              <span className="text-green-600 text-xl flex-shrink-0">✓</span>
              <span className="text-gray-700"><strong>Owners who value</strong> implementation tools over generic marketing theory</span>
            </div>
          </div>
        </div>

        {/* Implementation Options */}
        <div className="bg-white rounded-lg shadow-xl p-8 mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">
            📊 Implementation Options
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
            <div className="text-center p-6 bg-gray-50 rounded-lg border-2 border-gray-200">
              <div className="font-bold text-primary-700 mb-2 text-lg">DIY</div>
              <div className="text-3xl font-bold mb-2">$997</div>
              <div className="text-sm text-gray-600">Complete package<br/>Self-implementation</div>
            </div>
            <div className="text-center p-6 bg-primary-50 rounded-lg border-2 border-primary-300">
              <div className="font-bold text-primary-700 mb-2 text-lg">Guided</div>
              <div className="text-3xl font-bold mb-2">$2,997</div>
              <div className="text-sm text-gray-600">Package + setup call<br/>90-day check-ins</div>
            </div>
            <div className="text-center p-6 bg-green-50 rounded-lg border-2 border-green-400">
              <div className="font-bold text-green-700 mb-2 text-lg">Accelerated</div>
              <div className="text-3xl font-bold mb-2">$4,997</div>
              <div className="text-sm text-gray-600">Full training<br/>Team implementation</div>
            </div>
            <div className="text-center p-6 bg-yellow-50 rounded-lg border-2 border-yellow-400">
              <div className="font-bold text-orange-700 mb-2 text-lg">VIP</div>
              <div className="text-3xl font-bold mb-2">$7,997+</div>
              <div className="text-sm text-gray-600">Done-with-you<br/>Custom consulting</div>
            </div>
          </div>
          <p className="text-center text-sm text-gray-600 mt-6">
            Exact pricing depends on your store size, team structure, and implementation needs.
          </p>
        </div>

        {/* Final CTA */}
        <div className="bg-gradient-to-br from-primary-700 to-primary-900 text-white rounded-lg shadow-xl p-8 mb-8">
          <div className="text-center">
            <h2 className="text-4xl font-bold mb-6">🚀 Next Step: Schedule a Discovery Call</h2>
            
            <div className="bg-primary-800 rounded-lg p-6 mb-8 max-w-3xl mx-auto">
              <h3 className="font-bold text-white text-xl mb-4">Here's how it works:</h3>
              <ol className="text-left space-y-3 text-primary-100">
                <li className="flex items-start gap-3">
                  <span className="font-bold text-yellow-400 flex-shrink-0">1.</span>
                  <span><strong className="text-white">Schedule a 20-minute call</strong> (no pressure, no pitch)</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="font-bold text-yellow-400 flex-shrink-0">2.</span>
                  <span><strong className="text-white">We'll discuss</strong> your current review count, lead volume, and pain points</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="font-bold text-yellow-400 flex-shrink-0">3.</span>
                  <span><strong className="text-white">I'll recommend</strong> which package fits your situation (DIY, Guided, Accelerated, or VIP)</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="font-bold text-yellow-400 flex-shrink-0">4.</span>
                  <span><strong className="text-white">You decide</strong> if it makes sense for your business</span>
                </li>
              </ol>
            </div>
            
            <div className="mb-8">
              <a
                href="mailto:matt@mattshelton.co?subject=Discovery Call - Review System"
                className="bg-yellow-400 text-gray-900 px-10 py-5 rounded-lg font-bold hover:bg-yellow-300 transition-colors text-xl inline-flex items-center justify-center gap-2 shadow-lg"
              >
                📅 Schedule Your Discovery Call →
              </a>
            </div>

            <div className="border-t-2 border-primary-600 pt-6 max-w-2xl mx-auto">
              <p className="text-primary-100 mb-4 italic">Or, if you're ready to get started immediately, purchase the DIY package below.</p>
              <a
                href="mailto:matt@mattshelton.co?subject=Purchase DIY Package - Review System"
                className="bg-white text-primary-700 px-8 py-4 rounded-lg font-bold hover:bg-primary-50 transition-colors text-lg inline-flex items-center justify-center gap-2"
              >
                💳 Purchase DIY Package ($997) →
              </a>
            </div>

            <div className="text-sm text-primary-200 mt-8">
              <p>📧 Email: <a href="mailto:matt@mattshelton.co" className="underline hover:text-white font-semibold">matt@mattshelton.co</a></p>
              <p className="mt-1">⏱️ Typical response time: Within 24 hours</p>
            </div>
          </div>
        </div>

        {/* Footer Note */}
        <div className="mt-8 text-center text-sm text-gray-500">
          <p>© 2024 Matt Shelton Consulting. All rights reserved.</p>
          <p className="mt-2">Questions? Email matt@mattshelton.co</p>
        </div>
      </main>
    </div>
  );
}
